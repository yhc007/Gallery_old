...
<script>
	...
	function fncListStaffData(no){
		...
		var param = jQuery('#listStaffForm1').serialize() + "&shopId2=" + $("#shopIdForm select[id='shopId']").val();
		 ...
		 $.ajax({
			...
			success: function(data){
				if(data.result=="fail"){
					...
					var chkStaffId = data.staffId;
					$('#staffId').val(chkStaffId);
					alert("등록하였습니다.");
					fncAddStaffPhoto();
					//성공시....
					//fncStaffDetailClear();
					...
					//console.log("cnt:"+cnt);
					//if(cnt>0){
					//	startSubmitFn(data.staffId);
					//}
				}else if(data.result=="modified"){
					alert("수정되었습니다.");
					fncAddStaffPhoto();
					//var cnt = jQuery('#queuedFiles li').length;
					//if(cnt>0){
					//	startSubmitFn($('#staffId').val());
					//}
				}else{
					fncAddStaffPhoto();
					//var cnt = jQuery('#queuedFiles li').length;
				...
				success		: function(data){
					...
					jQuery('#listStaffForm2 select[name=shopId]').val(data.shopId);
		 			...
		 			if(data.imgPath!=null){
		 				document.listStaffForm2.staffImg.src=data.imgPath;
	 ...
	 */
	var chkFileName="";
	function fncAddStaffPhoto(){
		if(chkFileName != null ){
			var form = new FormData(document.getElementById('listStaffForm2'));
			$.ajax({ url: "${ctxPath}/staff/addStaffPhotoAction.do",
					 data: form,
			    dataType: 'text',
			 processData: false,
			 contentType: false,
...
<body>
	<div id="content">
		...
		<form name="listStaffForm1"  id="listStaffForm1" method="post" action="">
			...
			<form action="" id="shopIdForm">
				<select id="shopId" onchange="fncListStaffData(1)">
			...
			<table width="100%" border="1" class="detail">
				...
				<tr>
					...
					<td>
						...
						<input type="file" id="queuedFiles"  name="queuedFiles"  onchange="checkFile(this)" placeholder="사진 선택" />
					</td>
				</tr>
				<tr>
					<th>사진</th>
					<td colspan="3" align="center" valign="center">
						<div id="img_div">
							<img id="staffImg" width=200 class="round-image"/> <a href="#" onclick="fncDelImg();return false;">삭제</a>
						</div>
					</td>
				</tr>
			</table>
