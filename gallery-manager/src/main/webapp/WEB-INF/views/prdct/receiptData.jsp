...
<center>
	...
	<c:choose>
		<c:when test="true">
			<c:forEach var="prdct" items="${listRecepit }"> 
				<tr >
					<c:set var="dueMonth" value="${prdct.dueMonth }"></c:set>
					<c:set var="adminAllow" value="${prdct.adminAllow }"></c:set>
					<c:set var="thisMonth" value="${fn:substring(prdct.updTime,6,1)}"></c:set>
					...
					<c:set var="t" value="${prdct.puchasPrc * 0.1}"></c:set>
					
					<c:set var="t_" value="${t+((t%1>=0.5)?(1-(t%1))%1:-(t%1)) }"></c:set>
					<c:set var="tax" value="${t_*prdct.cnt}"></c:set>
					
					
					<c:choose>
						<c:when test="${adminAllow==1 and thisMonth <= dueMonth }">
							<td align="right" width="14%"><%-- 할부 (${prdct.devide } 개월)  --%><fmt:formatNumber value="${sum / prdct.devide}" pattern="#,###"/>&nbsp;/&nbsp;<fmt:formatNumber value="${sum}" pattern="#,###"/></td>
							<td align="right" width="14%"><fmt:formatNumber value="${tax / prdct.devide}" pattern="#,###"/>&nbsp;/&nbsp;<fmt:formatNumber value="${tax}" pattern="#,###"/></td>
							<c:set var="sum" value="${prdct.cnt * prdct.puchasPrc/prdct.devide}"></c:set>
							<c:set var="t" value="${prdct.puchasPrc * 0.1}"></c:set>
					
							<c:set var="t_" value="${t+((t%1>=0.5)?(1-(t%1))%1:-(t%1)) }"></c:set>
							<c:set var="tax" value="${t_*prdct.cnt/prdct.devide}"></c:set>
						</c:when>
						<c:otherwise>
			...
			<c:forEach var="prdct" items="${listRecepitLens2 }"> 
				<tr >
					<c:set var="dueMonth" value="${prdct.dueMonth }"></c:set>
					<c:set var="adminAllow" value="${prdct.adminAllow }"></c:set>
					<c:set var="thisMonth" value="${fn:substring(prdct.updTime,6,1)}"></c:set>
					...
					<td width="10%">${date }</td>
					<td width="30%" align="left">${prdct.prdctName } ${prdct.rate } </td>
					<td align="right" width="8%"><fmt:formatNumber value="${prdct.cnt}" pattern="#,###"/>EA</td>
					<td align="right" width="14%"><fmt:formatNumber value="${prdct.puchasPrc}" pattern="#,###"/></td>
					<c:set var="sum" value="${prdct.cnt * prdct.puchasPrc}"></c:set>
					<c:set var="t" value="${prdct.puchasPrc * 0.1}"></c:set>
	...
	<c:forEach var="prdct" items="${getRtnFrame }"> 
			...
			<c:forEach var="prdct" items="${getRtnLens2 }"> 
				<tr >
					<c:set var="day" value="${prdct.updTime}"></c:set>
					<c:set var="date" value="${fn:substring(day,5,10)}"></c:set>
					<td width="10%">${date }</td>
