...
<script type="text/javascript">
	...
	$(function() {
		...
		window.sessionStorage.setItem("menu","clens");
		var date = new Date();
	...
	function getClensTyList(){
		...
		newTy1 = false;
		newTy2 = false;
		$("#ty1").css("display","inline");
		$("#newTy1").css("display","none");
		$("#ty2").css("display","inline");
		$("#newTy2").css("display","none");
	...
	function getClensTy2List(){
		...
		var param = "brandId=" + brandId + "&tyId1=" + ty1 + "&allPrdct=" + allPrdct + "&shopTy=shop"; 
		$.ajax({
			url		: url,
			data : param, 
			type 	: "post",
	...
	function addNewPrdct(){
		...
		var url = "${ctxPath}/invn/addShopClens.do";
		$.ajax({
			url : url,
			type : "post",
			data : param,
	...
	function addPrdct(pId){
		...
		if(newPrdct || newTy1 || newTy2){ //새로운 prdctName
			addNewPrdct();
			return;
		}
		var shopId = '${shopId}';		
		...
		var url = '${ctxPath}/prdct/addClens.do';
	...
	function	getPrdctPrc(){
		...
		var url = '${ctxPath}/prdct/getClensPrc.do';
		 $.ajax({
				url: url,
				type : "post",
				data : param,
...
<center>
	<form id="PrdctInfo" class="frameTbl">
		<table id="container" width="80%"border="1" style="border-collapse: collapse; text-align: center" >
			...
			<tr>
				<th>브랜드</th><td><select id="brandId" name="brandId" onchange="getClensTyList();">
										<option value="-1">선택</option>
									</select></td>
				<th>타입</th><td><select id="ty1" name="ty1" onchange="getClensTy2List();">
										<option value="-1">선택</option>
									</select><input type="text" id="newTy1"> </td>
				
			</tr>
			<tr>
			<th>기능</th><td><select id="ty2" name="ty2" onchange="getPrdctList();">
										<option value="-1">선택</option>
									</select>
									<input type="text" id="newTy2">
									</td>
				
				<th width="20%">모델명</th><td width="30%"> <select id="prdctId" name="prdctId" onchange="getPrdctPrc()"><option value="-1">선택</option></select><input type="text" id="newPrdctName" class="test"></td>
