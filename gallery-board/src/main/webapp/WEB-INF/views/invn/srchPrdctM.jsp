...
<script type="text/javascript">
	$(function(){
		window.sessionStorage.setItem("menu","srch");
	...
	function insertDiffClr(){
		...
		var param = $("#PrdctInfo").serialize() + "&brandId=" + brandId+ "&prdctName=" + prdctName + "&prdctTyCd=" + prdctTyCd + "&shopId=${shopId}" + "&invnHistId=" + invnHistId + "&prdctId=" + prdctId + 
					"&invnTyCd=" + invnTyCd + "&shopTy=shop";
	...
	function editInvnData(){
		...
		var param = $("#PrdctInfo").serialize() + "&invnHistId=" + invnHistId + "&datetime=" + removeHypen($("#datetime").val()) +
			"&prdctId=" + prdctId + "&prdctName=" + prdctName + "&brandId=" + brandId + "&shopTy=shop";
		//alert("modify param : " + param)
		$.ajax({
			url : url,
			type : "post",
			data : param,
	...
	function getBrandByTy(){
		...
		var param = "prdctTyCd=" + $("#prdctTyCd").val() + "&shopTy=shop";
	...
	function delHist(histId){
		var url = "${ctxPath}/prdct/delHistData.do";
...
<center>
	<form id="PrdctInfo">
		<table id="container" width="80%"border="1" style="border-collapse: collapse; text-align: center" >
			...
			<tr>
				<th width="20%">브랜드</th><td width="30%"><select id='brandId' name='brandId' title='브랜드 명' onchange="getPrdctList();" disabled="disabled"></select></td>
				<th width="20%">모델명</th><td width="30%"> <select id="prdctId" name="prdctId" onchange="getPrdctPrc()" disabled="disabled"><option value="-1">선택</option></select></td>
			</tr>
			<tr>
