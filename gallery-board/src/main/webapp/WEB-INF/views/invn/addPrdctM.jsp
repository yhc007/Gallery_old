...
<script type="text/javascript">
	...
	function addPrdct(pId){
		...
		+ "&mnfCountry=" + mnfCountry + "&colorId=" + colorId + "&colorId2=" + colorId2 +"&iNum=" + iNum + "&puchasPrc=" + puchasPrc + "&trdePrc=" + trdePrc + "&prdctVisibleCd=00500002" + "&shopTy=shop";
	...
	if(!newPrdct && newName!= Name){
		...
		var param = "shopId=${shopId}" + "&prdctName=" + prdct + color + "&brandId=" + brandId + "&shopTy=shop";
		$.ajax({
			url : url,
	...
	function getBrand(){
		...
		var param = "brandName=" + brandName + "&shopTy=shop";
	...
	function newBrand(){
		if(!newBrandN){
			...
			$("#newBrand").css("display","inline");
			$("#addBrand").html("추가");	
			newBrandN = true;
			newBrandId = "";
		}else{
			var param = "brandName=" + $("#newBrand").val();
			var url = "${ctxPath}/brand/addNewBrand.do";
...
<center>
	<form id="PrdctInfo" class="frameTbl">
		<table id="container" width="80%"border="1" style="border-collapse: collapse; text-align: center" >
			<tr>
				<th>브랜드검색</th>
				<td><input type="text" id="srchBrand" onkeyup="getBrand(); return false;"></td>
			...
			<tr>
				<th width="20%">브랜드</th><td width="30%"><select id='brandId' name='brandId' title='브랜드 명' onchange="getPrdctList();">
							...
							<input id="newBrand"><br><a href="javascript:newBrand()" id="addBrand">직접입력</a>
							</td>
				<th width="20%">모델명</th><td width="30%"> <input type="text" placeholder="모델명 검색" id="srchPrdct"  onkeyup="getPrdct(); return false;"><br> <select id="prdctId" name="prdctId" onchange="getPrdctPrc()"><option value="-1">선택</option></select><input type="text" id="prdctName_" class="test"></td>
			</tr>
			<tr>
