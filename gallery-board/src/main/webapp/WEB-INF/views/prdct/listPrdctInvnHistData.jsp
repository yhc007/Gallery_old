	...
	<c:choose>
		<c:when test="${!empty listPrdct}">
	   		<c:forEach var="prdct" items="${listPrdct}" varStatus="status">
				<tr onclick="fncGetPrdctInvnHistInfo('${prdct.invnHistId}');return false;" class="listData">
					<!-- 
				    ...
				    <td align="center">${prdct.cstmrId}</td>
				    <td align="center">${prdct.regTime}</td>
				</tr>			
			</c:forEach>
		...
		<c:otherwise>
			<tr>					
				<td colspan="9" align="center">이력 데이터가 없습니다.</td>	
			</tr>
		</c:otherwise>
	</c:choose>
</table>
