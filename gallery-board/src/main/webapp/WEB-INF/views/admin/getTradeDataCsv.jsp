	...
	<c:choose>
		<c:when test="${!empty trdeList}">
			...
			<c:forEach var="trde" items="${trdeList}" varStatus="status">
				  ...
				  <c:if test="${trde.comName != comName}">
				  <c:if test="${init ==1 }">
					  ...
					  <c:if test="${number<10 }">
						...
						<table width="90%">
							...
							<c:set var="number" value="1"></c:set>
							<tr>
								<td style="font-weight: bold; font-size: 17px">${trde.comName }</td>
								<td></td>
								<td></td>
								<td></td>
						...
						<c:set var="endDiv" value="</div>"></c:set>
							<thead>
								<tr>
									...
									<td width="16%" style="text-align: center">매장</td>
									<td width="16%" style="text-align: center">매출금액</td>
									<td width="16%" style="text-align: center">반품금액</td>
									<td width="16%" style="text-align: center">결재금액</td>
									<td width="16%" style="text-align: center">할부금액</td>
									<td width="16%" style="text-align: center">수수료</td>		
								</tr>
							</thead>
							<c:set var="comName" value="${trde.comName }"></c:set>			
					</c:if>
					...
					<c:if test="${trde.comName == comName}">
						<tbody>
							<tr>
								...
								<td style="text-align: center">${trde.shopName }</td>
								...
								<td style="text-align: right;padding-right: 10px;"><fmt:formatNumber value="${trde.tax}" pattern="#,###" /></td>
								...
								<c:set var="tax" value="${tax + trde.tax}"></c:set>
